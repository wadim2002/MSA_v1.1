...
def post_writemq(text):
    ...
    channel.basic_publish(exchange='', routing_key='Post', body=text)    
    connection.close()
    
    return True

def post_CountNoRead(text):
    # Устанавливаем соединение с сервером RabbitMQ
    connection = pika.BlockingConnection(pika.ConnectionParameters('rabbitmq',5672))#("amqp://guest:guest@rabbitmq:5672/vhost"))
    channel = connection.channel()
    ...
    # Отправляем сообщение в очередь
    body_txt = '{"id": "", "user" : "0","operation" : "CounterNoReadPost", "text" : "-" , "status" : "-" , "value" : "' + str(text) + '" }'
    channel.basic_publish(exchange='', routing_key='OUT',body=body_txt)    
    connection.close()
    print (body_txt)
    return True